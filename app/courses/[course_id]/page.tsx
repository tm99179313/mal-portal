...
export default function CourseStudyPage({ params }: { params: Promise<{ course_id: string }> }) {
  ...
  // 🛑 【追加】モーダルが開いている間は、背景（body）のスクロールを禁止する
  useEffect(() => {
    if (activeSession) {
  ...
  const fetchSessions = async (id: string) => {
    const { data: s } = await supabase.from('sessions').select('*').eq('course_id', id).order('order_index', { ascending: true });
    setSessions(s || []);
  };
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-10 max-w-md w-full shadow-xl relative z-10">
          <div className="flex justify-center mb-6">
            <div className="w-16 h-16 bg-slate-50 border border-slate-100 rounded-full flex items-center justify-center shadow-inner">
              <svg className="w-8 h-8 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8V7z"></path></svg>
            </div>
          </div>
          <h2 className="text-2xl font-bold text-center text-slate-800 mb-2">{course?.name}</h2>
          <p className="text-sm text-center text-slate-500 mb-8">このコースを学習するにはパスコードを入力してください。</p>
          ...
          <form onSubmit={handleUnlock} className="space-y-4">
            ...
            {errorMsg && <p className="text-red-500 text-xs font-bold text-center bg-red-50 py-2 rounded">{errorMsg}</p>}
            <button type="submit" className="w-full bg-blue-600 text-white font-bold py-4 rounded-lg hover:bg-blue-500 active:scale-[0.98] transition-all shadow-md flex justify-center items-center gap-2">
              ロックを解除
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
            </button>
          </form>
          <div className="mt-6 text-center">
            <Link href="/courses" className="text-sm text-slate-400 hover:text-slate-600 font-bold transition-colors">← コース一覧に戻る</Link>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800">
      ...
      <main className="max-w-4xl mx-auto p-6 md:p-12">
        <div className="mb-10">
          <h1 className="text-3xl md:text-4xl font-bold text-slate-800 mb-3">{course?.name}</h1>
          <p className="text-slate-500 font-medium">学習するセッションを選択してください</p>
        </div>

        <div className="space-y-4">
          {sessions.length === 0 ? (
            <div className="bg-white border border-slate-200 rounded-xl p-10 text-center shadow-sm">
              <p className="text-slate-400 font-bold">まだセッションが登録されていません。</p>
            </div>
          ) : (
            sessions.map((s, i) => (
              <button 
              ...
              >
                ...
                <div className="flex-grow">
                  <h3 className="text-xl font-bold text-slate-800 group-hover:text-blue-600 transition-colors">{s.title}</h3>
                  <div className="flex flex-wrap gap-2 mt-2">
                    {s.video_url && <span className="text-xs font-bold text-slate-500 bg-slate-100 px-2 py-1 rounded">🎬 動画収録あり</span>}
                    {s.zoom_url && <span className="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded">🔵 Zoomライブ</span>}
                    {s.pdf_url && <span className="text-xs font-bold text-slate-500 bg-slate-100 px-2 py-1 rounded">📄 配布資料あり</span>}
                  </div>
                </div>
            ...
            <div className="overflow-y-auto flex-grow bg-black">
              {getYouTubeEmbedUrl(activeSession.video_url) ? (
                ...
                <div className="w-full aspect-video bg-slate-900 flex flex-col items-center justify-center p-10 text-center border-b border-slate-800">
                  ...
                  <h4 className="text-2xl font-bold text-white mb-3">Zoom ライブセッション</h4>
                  <p className="text-slate-400 mb-8 max-w-md leading-relaxed">
                    この回はZoomでのリアルタイム配信となります。<br />お時間になりましたら、下のボタンから参加してください。
                  </p>
                  <a href={activeSession.zoom_url} target="_blank" rel="noopener noreferrer" className="bg-blue-600 text-white px-8 py-4 rounded-lg font-bold text-lg hover:bg-blue-500 active:scale-95 transition-all shadow-[0_0_20px_rgba(37,99,235,0.4)]">
                    Zoomを起動する
                  </a>
                </div>
              ) : (
                <div className="w-full aspect-video bg-slate-900 flex items-center justify-center border-b border-slate-800">
                  <p className="text-slate-600 font-bold">コンテンツが準備中です</p>
                </div>
              )}

              <div className="p-6 md:p-8 bg-slate-950 flex flex-col md:flex-row items-center justify-between gap-6">
                <div>
                  <h4 className="text-slate-300 font-bold text-sm mb-1">セッション配布資料</h4>
                  <p className="text-slate-500 text-xs">動画やライブ配信に合わせてご活用ください。</p>
                </div>
                
                <div>
                  {activeSession.pdf_url ? (
                    <a href={activeSession.pdf_url} target="_blank" rel="noopener noreferrer" className="bg-slate-800 border border-slate-700 text-white px-6 py-3 rounded-lg font-bold hover:bg-slate-700 hover:border-slate-500 active:scale-95 transition-all flex items-center gap-2">
