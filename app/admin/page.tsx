      ...
      <div className="relative z-10 max-w-5xl mx-auto p-8 md:p-12">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 border-b border-slate-200 pb-6 gap-4 md:gap-0">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
            ...
            <Link
              ...
              className="flex-1 md:flex-none text-sm font-bold text-indigo-700 bg-indigo-50 border border-indigo-200 px-4 py-2.5 rounded-md shadow-sm hover:bg-indigo-100 hover:text-indigo-800 hover:border-indigo-300 active:scale-95 transition-all flex items-center justify-center gap-2 group"
            >
              <svg
                className="w-4 h-4 text-indigo-400 group-hover:text-indigo-600 transition-colors"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            >
              <span className="relative z-10 flex items-center justify-center gap-2">
                <span className="font-mono text-blue-200 opacity-70">+</span>
                新規追加
              </span>
        ...
        <div className="space-y-6">
          {courses.map((course) => (
            <div
              key={course.id}
              className="relative bg-white border border-slate-200 rounded-lg p-6 flex flex-col md:flex-row md:items-center justify-between shadow-sm hover:shadow-[0_0_20px_rgba(37,99,235,0.1)] hover:border-blue-300 transition-all duration-300 group overflow-hidden"
            >
                ...
                <div className="flex-grow pl-2 pr-6 mb-4 md:mb-0">
                  ...
                  <div className="mt-3">
                    ...
                    <input
                      ...
                      className="bg-slate-50 border border-blue-300 p-2 rounded w-full font-bold focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                ...
                <div className="mb-4 md:mb-0 pl-2 flex-grow">
                  ...
                  >
                    <h2 className="text-2xl font-bold text-slate-800 mb-2 group-hover/link:text-blue-600 transition-colors flex items-center gap-2">
                      {course.name}
                      <svg
                        className="w-5 h-5 text-slate-300 group-hover/link:text-blue-500 transition-colors opacity-0 group-hover/link:opacity-100 transform -translate-x-2 group-hover/link:translate-x-0"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
                ...
                {editingCourseId === course.id ? (
                  <>
                    <button
                      ...
                      className="bg-emerald-500 text-white border border-emerald-600 px-5 py-2 rounded-md font-bold hover:bg-emerald-600 active:scale-95 transition-all shadow-sm"
                    >
                      保存
                    </button>
                    <button
                      onClick={() => setEditingCourseId(null)}
                      className="bg-slate-200 text-slate-700 border border-slate-300 px-5 py-2 rounded-md font-bold hover:bg-slate-300 active:scale-95 transition-all shadow-sm"
                    >
                      取消
                    </button>
                  </>
                  ...
                  <>
                    <button
                      ...
                      className="border border-slate-300 text-slate-700 px-5 py-2 rounded-md font-semibold hover:bg-slate-50 hover:border-blue-300 hover:text-blue-600 active:scale-95 transition-all"
                    >
                      コース編集
                    </button>
                    <button
                      onClick={() => handleDelete(course.id)}
                      className="border border-red-200 text-red-600 px-5 py-2 rounded-md font-semibold hover:bg-red-50 hover:border-red-300 active:scale-95 transition-all"
                    >
                      削除
                    </button>
                  </>
