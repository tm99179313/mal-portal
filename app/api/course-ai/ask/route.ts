...
　　　　　　　'回答は原則800〜1200字以内にしてください。',
　　　　　　　'長くなる場合は、文章を伸ばさず箇条書き中心で簡潔に整理してください。',
　　　　　　　'質問が「詳しく」「網羅的に」などの場合のみ、必要に応じて1500字程度まで許可します。',
...
　　　　　　　'治療方針を断定せず、最終判断は担当歯科医師が行う前提で表現してください。',
        ...
        },
        {
          ...
          content: [
           ...
           '【回答時の追加ルール】',
　　　　　　'- まず短く結論を出してください。',
　　　　　　'- 原則800〜1200字以内で回答してください。',
　　　　　　'- 1文を長くしすぎず、箇条書きを中心にしてください。',
...
export async function POST(req: Request) {
  try {
    ...
    const limitCheck = await checkDailyQuestionLimit(user.email);

if (!limitCheck.allowed) {
  return NextResponse.json(
    {
      error: `本日のAI質問回数が上限に達しました。通常ユーザーは1日${limitCheck.limit}回まで利用できます。また明日ご利用ください。`,
      countToday: limitCheck.countToday,
